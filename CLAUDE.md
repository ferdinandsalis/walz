...
This is the website for Walz, a school in Vienna run by a friend. This project
has been developed entirely by hand without AI assistance until now. The site is
built with React Router v7 (previously Remix) and uses Sanity CMS for content
management. The codebase is written in TypeScript and uses Tailwind CSS for
styling.

## Development Commands

- `npm run dev` - Start development server
- `npm run build` - Build production application (runs both React Router and
  server builds)
...
Uses flat routes convention with route files in `app/routes/`. Special route
patterns:
...
The site content is managed through Sanity CMS with the following main content
types:
