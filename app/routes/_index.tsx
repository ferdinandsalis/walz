...
const faq = [
  {
    question: 'Was heißt eigentlich die Walz?',
    answer:
    ...
    <div className="space-y-16 md:space-y-20 lg:space-y-24">
      <div className="relative -mx-8 grid grid-cols-1 grid-rows-1 md:-mx-12">
        ...
        <div className="relative col-start-1 row-start-1 flex flex-col items-start justify-between bg-black/40 p-12 sm:p-16 md:p-20 lg:py-24">
          <p className="max-w-xl font-sans text-4xl font-bold tracking-tight text-white sm:text-5xl md:text-5xl lg:text-6xl">
            ...
            darauf vor&shy;bereiten, mit Mut und Liebe die Welt und
            Gesell&shy;schaft mitzuge&shy;stalten.
          </p>
          <LogoSymbol className="absolute -bottom-32 -right-10 w-56 text-primary opacity-70 sm:w-72 md:w-80 lg:right-10 lg:w-96" />
        </div>

        <article className="col-start-1 row-start-2 flex items-center gap-2 bg-neutral-200 p-3 px-4 shadow-inner sm:px-8 md:px-12">
      ...
      <section>
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          <Link
            to="./schule/philosophie/bildung"
            className="group rounded-lg bg-white p-6 shadow-md"
          >
            <div className="relative mb-6">
              <img
                src=""
                className="ascpet-square h-48 w-full border-4 border-secondary bg-neutral-100 object-cover"
              />
            </div>
            <h1 className="font-condensed text-2xl font-bold text-primary md:text-3xl lg:text-4xl xl:text-5xl">
              Bildung
            </h1>
            <h2 className="font-condensed text-2xl font-bold text-neutral-700">
          ...
          <Link
            to="./schule/philosophie/persoenlichkeit"
            className="group rounded-lg bg-white p-6 shadow-md"
          >
            <div className="relative mb-6">
              <img
      ...
      <section>
        ...
        {testimonials.map((entry, idx) => {
          return (
            ...
            >
              <p className="relative text-lg font-light italic text-neutral-700 md:text-xl lg:text-2xl">
                {entry.text}
              </p>
              <footer className="mt-4">
                <p className="font-condensed text-lg font-bold text-neutral-700 md:text-xl lg:text-2xl">
                  —&thinsp;{entry.name}
                </p>
              </footer>
            </blockquote>
      ...
      <section>
        <header className="mb-16 lg:mb-20">
          <SectionHeading id="kontakt">Addresse & Kontakt</SectionHeading>
        </header>
        <div className="grid grid-cols-6 grid-rows-2 gap-8">
          ...
          <div className="col-span-4 col-start-1 row-start-2 overflow-hidden rounded border border-neutral-300 bg-neutral-200 shadow-inner shadow-neutral-300 ring-1 ring-neutral-50 lg:row-span-2 lg:row-start-1">
            <iframe
              width="100%"
              height="100%"
              title="Felt Map"
...
function SectionHeading({ children, id }: { children: any; id: string }) {
  return (
    <Link to={`.#${id}`} className="grid grid-cols-12 items-center">
      <h1
        id={id}
        className="col-span-1 col-start-1 col-end-13 row-start-1 row-end-1 text-center font-condensed text-base font-bold uppercase tracking-[0.2em] text-neutral-400"
      >
        <span className="rounded bg-neutral-100 px-8 py-2">{children}</span>
      </h1>
      <hr className="-order-1 col-start-1 col-end-13 row-start-1 h-[4px] rounded border-none bg-neutral-200/70" />
    </Link>
  )
}
